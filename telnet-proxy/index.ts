...
server.on('connection', (socket: ws.WebSocket, req: IncomingMessage) => {
    ...
    function ondata(chunk: Chunk) {
        ...
        switch (chunk.type) {
            case 'DATA':
                const string = new TextDecoder(charset.encoding).decode(
                    chunk.data,
                )
                socket.send(string)
                return
                ...
                switch (chunk.target) {
                    case Cmd.TERMINAL_SPEED:
                        if (chunk.target === Cmd.WILL) {
                            console.log(
                                'sending IAC DONT TERMINAL_SPEED to server',
                            )
                            telnet.write(
                                Uint8Array.from([
                                    Cmd.IAC,
                    ...
                    case Cmd.NEW_ENVIRON:
                        if (chunk.target === Cmd.WILL) {
                            console.log(
                                'sending IAC DONT NEW_ENVIRON to server',
                            )
                            telnet.write(
                                Uint8Array.from([
                                    Cmd.IAC,
                    ...
                    case Cmd.ECHO:
                        if (chunk.target === Cmd.WILL) {
                            ...
                            telnet.write(
                                Uint8Array.from([Cmd.IAC, Cmd.DO, Cmd.ECHO]),
                            )
                        }
                        return
                    case Cmd.MCCP2:
                        if (chunk.target === Cmd.WILL) {
                            ...
                            telnet.write(
                                Uint8Array.from([Cmd.IAC, Cmd.DO, Cmd.MCCP2]),
                            )
                    ...
                    case Cmd.GMCP:
                        if (chunk.target === Cmd.WILL) {
                            ...
                            telnet.write(
                                Uint8Array.from([Cmd.IAC, Cmd.DO, Cmd.GMCP]),
                            )
                        }
                        return
                    default:
